...
resource "aws_ebs_volume" "hana_data" {
  count             = var.hana_data_amount != null ? var.hana_data_amount : local.hana_data[var.instance_type][var.hana_data_type].amount
  availability_zone = aws_instance.instance.availability_zone
  size              = var.hana_data_size != null ? var.hana_data_size : local.hana_data[var.instance_type][var.hana_data_type].disk_size
...
resource "aws_volume_attachment" "ebs_hana_data_att" {
  count = var.hana_data_amount != null ? var.hana_data_amount : local.hana_data[var.instance_type][var.hana_data_type].amount
  device_name = element(local.hana_data_device_names, count.index)
  volume_id   = element(aws_ebs_volume.hana_data.*.id, count.index)
  instance_id = aws_instance.instance.id
}

resource "aws_ebs_volume" "hana_log" {
  count             = var.hana_log_amount != null ? var.hana_log_amount : local.hana_log[var.instance_type][var.hana_log_type].amount
  availability_zone = aws_instance.instance.availability_zone
  size              = var.hana_log_size != null ? var.hana_log_size : local.hana_log[var.instance_type][var.hana_log_type].disk_size
...
resource "aws_volume_attachment" "ebs_hana_log_att" {
  count = var.hana_log_amount != null ? var.hana_log_amount : local.hana_log[var.instance_type][var.hana_log_type].amount
  device_name = element(local.hana_log_device_names, count.index)
...
resource "aws_ebs_volume" "hana_shared" {
  availability_zone = aws_instance.instance.availability_zone
  size              = var.hana_shared_size != null ? var.hana_shared_size : local.hana_shared[var.instance_type]
  type              = "gp2"
  encrypted         = true
  kms_key_id        = var.kms_key_id
